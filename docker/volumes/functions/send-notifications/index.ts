...
Deno.serve(async (req) => {
  try {
    ...
    // Query for all followers of the given user
    const { data: followers, error } = await supabaseClient
      .from("followed_friends")
      .select("friend_id")
      .eq("user_id", userId);
    ...
    const followerIds = followers.map((follower) => follower.friend_id);

    if (followerIds.length === 0) {
      return new Response(JSON.stringify({ error: "No followers found" }), {
        status: 400,
      });
    }
